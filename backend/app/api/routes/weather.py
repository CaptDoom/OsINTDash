...
async def get_border_weather():
    ...
    if not api_key:
        ...
        weather_cache["expires_at"] = now + 600.0 # 10 min cache
        return weather_data
        ...
        for name, base in SECTORS.items():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    wind_speed = owm_data.get("wind", {}).get("speed", base["base_wind"] / 3.6) * 3.6 # convert m/s to km/h
                    ...
                    # Fallback to base data if this coordinate fails
                    weather_data[name] = {
                        "sector": name,
                        "latitude": base["lat"],
                        "longitude": base["lon"],
